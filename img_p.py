...
while (count <= 5000):
	img = cv2.imread('img/'+str(count)+'.jpg')
	...
	plt.savefig('img_p/'+str(count)+'.jpg',dpi=10)
	img2 = cv2.imread('img_p/'+str(count)+'.jpg')
	...
	plt.close() #close figure of origin size
	plt.figure(figsize=(140, 48), dpi=100) #存成固定大小 (w, h)
	plt.axis('off')
	plt.imshow(newdst)

	plt.subplots_adjust(top=1, bottom=0, left=0, right=1, hspace=0, wspace=0)#去白邊
	plt.savefig('img_p/'+str(count)+'.jpg',dpi=1)
	plt.close() #close figure of origin size
